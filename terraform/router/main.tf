module "cloudfront_apigw_lambda" {
  ...
  cloudfront_apigw_cloudfront_function_name     = "NightbearRouter"
  cloudfront_apigw_lambda_domain_name           = "${var.router_domain_name}"
  cloudfront_apigw_cloudfront_function_filename = "${path.module}/lambda-multicast-proxy-1.0.1.zip" # https://github.com/jareware/lambda-multicast-proxy
  cloudfront_apigw_lambda_domain_zone           = "${var.router_domain_zone}"

  cloudfront_apigw_cloudfront_function_env_vars = {
