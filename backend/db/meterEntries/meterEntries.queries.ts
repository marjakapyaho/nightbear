...
const upsertMeterEntryIR: any = {
  usedParamSet: { timestamp: true, bloodGlucose: true },
  params: [
    { name: 'timestamp', required: true, transform: { type: 'scalar' }, locs: [{ a: 70, b: 80 }] },
    {
      name: 'bloodGlucose',
      required: true,
 ...
 * ```
 */
export const upsertMeterEntry = new PreparedQuery<IUpsertMeterEntryParams, IUpsertMeterEntryResult>(
  upsertMeterEntryIR,
)
...
const deleteMeterEntryIR: any = {
  usedParamSet: { timestamp: true },
  params: [
    { name: 'timestamp', required: true, transform: { type: 'scalar' }, locs: [{ a: 44, b: 54 }] },
  ],
  statement: 'DELETE FROM meter_entries\nWHERE timestamp = :timestamp!\nRETURNING timestamp',
}
 ...
 * ```
 */
export const deleteMeterEntry = new PreparedQuery<IDeleteMeterEntryParams, IDeleteMeterEntryResult>(
  deleteMeterEntryIR,
)
...
const createMeterEntriesIR: any = {
  usedParamSet: { meterEntries: true },
  params: [
    {
      name: 'meterEntries',
      required: false,
      transform: {
        ...
        keys: [
          { name: 'bloodGlucose', required: true },
 ...
 * ```
 */
export const createMeterEntries = new PreparedQuery<
  ICreateMeterEntriesParams,
  ICreateMeterEntriesResult
>(createMeterEntriesIR)
...
const getMeterEntriesByTimestampIR: any = {
  usedParamSet: { from: true, to: true },
  params: [
    { name: 'from', required: true, transform: { type: 'scalar' }, locs: [{ a: 74, b: 79 }] },
    { name: 'to', required: false, transform: { type: 'scalar' }, locs: [{ a: 107, b: 109 }] },
  ],
  statement:
 ...
 * ```
 */
export const getMeterEntriesByTimestamp = new PreparedQuery<
  IGetMeterEntriesByTimestampParams,
  IGetMeterEntriesByTimestampResult
>(getMeterEntriesByTimestampIR)
