...
import { PouchDbState, PouchDbStatePart, PouchDbStatus } from 'web/src/modules/pouchDb/state';
import { assertNumber, objectKeys } from 'web/src/utils/types';
...
export function getSummaryDbStatus(
  ...
  if (isNumber(summaryProgress)) {
    const summaryStatus = state.LOCAL.status === 'DISABLED' ? 'REPLICATION_INITIAL' : 'REPLICATION_CATCHUP';
    return {
      summaryStatus,
      summaryProgress,
...
function getSummaryReplicationProgress(parts: Array<PouchDbState[PouchDbStatePart]>): number | null {
  const tally = (index: 0 | 1) =>
    parts.reduce((memo, next) => memo + ((isArray(next.details) && assertNumber(next.details[index])) || 0), 0);
  const done = tally(0);
  const todo = tally(1);
